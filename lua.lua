...
local num = 42  -- 所有的数字都是双精度浮点型。
-- 别害怕，64位的双精度浮点型数字中有52位用于
-- 保存精确的整型值; 对于52位以内的整型值，
-- 不用担心精度问题。

local s = 'walternate'  -- 和Python一样，字符串不可变。
local t = "也可以用双引号"
local u = [[ 多行的字符串
       以两个方括号
       开始和结尾。]]
...
-- 未定义的变量返回nil。 
-- 这不是错误： 
local foo = anUnknownVariable  -- 现在 foo = nil.
...
-- 数字和字符串的key按值匹配的
-- table按id匹配。
local a = u['@!#']  -- 现在 a = 'qbert'.
local b = u[{}]     -- 我们或许期待的是 1729,  但是得到的是nil:
-- b = nil ，因为没有找到。
